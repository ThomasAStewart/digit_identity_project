...
# It will recover both the correlation between orthologs among the 3 species for genes differentially expressed at D3/D4  of alligator and also between the differentially expressed genes and random genes of similar expression level.
...
for(k in 1:length(limb)){
  ...
  foldchange_lists <- list(data.frame(tpm_fold_change_mm[,1:2], tpm_fold_change_mm["change2v3"]),
                           data.frame(tpm_fold_change_ac[,1:2], tpm_fold_change_ac["change2v3"]),
  ...
  target_list <- merge(TPM_withID, one_to_one_am_23, by="am_gene_id")
  target_list <- target_list[order(target_list$gene_symbol),]
  
  target_list_TPM <- target_list$TPM_am_2_e
  target_list_fold <- foldchange_lists[[3]]$am_gene_id %in% one_to_one_am_23$am_gene_id
  ...
  # real
  real_fold_list <- merge(one_to_one_am_23, foldchange_lists[[k]], by=taxonid, sort=F)
...
corr_table <- cbind (position="d2_d3", DE_source=rep("am_e", 2), comparison=c("random", "orthologs"), corr_table)
...
for(k in 1:length(limb)){
  ...
  target_list_TPM <- target_list$TPM_am_2_l
  target_list_fold <- foldchange_lists[[4]]$am_gene_id %in% one_to_one_am_23$am_gene_id
...
corr_table <- matrix(unlist(simulated_gene_matrix),nrow=2, ncol=4)
colnames(corr_table) <- c("vs_mm", "vs_ac", "vs_am_e", "vs_am_l")
corr_table <- cbind (position="d2_d3", DE_source=rep("am_l", 2), comparison=c("random", "orthologs"), corr_table)
