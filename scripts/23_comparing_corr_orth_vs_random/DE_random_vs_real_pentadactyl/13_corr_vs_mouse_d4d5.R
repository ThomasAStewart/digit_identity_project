...
# It will recover both the correlation between orthologs among the 3 species for genes differentially expressed at D4/D5 of mouse and also between the differentially expressed genes and random genes of similar expression level.
...
for(k in 1:length(limb)){
  ...
  target_list <- merge(TPM_withID, one_to_one_mm_45, by="mm_gene_id")
  target_list <- target_list[order(target_list$gene_symbol),]
  
  target_list_TPM <- target_list$TPM_mm_4
  target_list_fold <- foldchange_lists[[1]]$gene_symbol %in% one_to_one_mm_45$gene_symbol #### changed from ID.
  ...
  # real
  real_fold_list <- merge(one_to_one_mm_45, foldchange_lists[[k]], by=taxonid, sort=F)
  real_fold_list <- merge(one_to_one_mm_45, foldchange_lists[[k]], by=taxonid, sort=F)
...
corr_table <- cbind (position="d4_d5", DE_source=rep("mm", 2), comparison=c("random", "orthologs"), corr_table)
