...
# It will recover both the correlation between orthologs among the 3 species for genes differentially expressed at D1/D2 of mouse and also between the differentially expressed genes and random genes of similar expression level.
...
for(k in 1:length(limb)){
  ...
  target_list_fold <- foldchange_lists[[1]]$gene_symbol %in% one_to_one_mm_12$gene_symbol #### changed from ID.
  ...
  cor(simulated_gene_list[3], target_list_fold) # seems to show that the simulated one works...
