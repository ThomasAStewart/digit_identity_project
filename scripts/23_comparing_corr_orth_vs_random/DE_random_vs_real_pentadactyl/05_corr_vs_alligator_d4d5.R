...
# It will recover both the correlation between orthologs among the 3 species for genes differentially expressed at D4/D5 of alligator and also between the differentially expressed genes and random genes of similar expression level.
...
for(k in 1:length(limb)){
  ...
  target_list <- merge(TPM_withID, one_to_one_am_45, by="am_gene_id")
  target_list <- target_list[order(target_list$gene_symbol),]
  
  target_list_TPM <- target_list$TPM_am_4_e
  target_list_fold <- foldchange_lists[[3]]$am_gene_id %in% one_to_one_am_45$am_gene_id
