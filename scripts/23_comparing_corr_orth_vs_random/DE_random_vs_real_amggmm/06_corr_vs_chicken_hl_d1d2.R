...
for(k in 1:length(limb)){
  ...
  target_list <- merge(TPM_gg_withID, one_to_one_gg_12, by="gg_gene_id")
  target_list <- target_list[order(target_list$gene_symbol),]
  
  target_list_TPM <- target_list$gg_HL_D1_E
  target_list_fold <- foldchange_lists[[2]]$gg_gene_id %in% one_to_one_gg_12$gg_gene_id
  ...
  # real
  real_fold_list <- merge(one_to_one_gg_12, foldchange_lists[[k]], by=taxonid, sort=F)
...
corr_table <- cbind (position="d1_d2", DE_source=rep("gg_e", 2), comparison=c("random", "orthologs"), corr_table)
...
for(k in 1:length(limb)){
  ...
  target_list_TPM <- target_list$gg_HL_D1_L
  target_list_fold <- foldchange_lists[[3]]$gg_gene_id %in% one_to_one_gg_12$gg_gene_id
...
corr_table <- matrix(unlist(simulated_gene_matrix), nrow=2, ncol=5)
colnames(corr_table) <- c("vs_mm", "vs_gg_e", "vs_gg_l", "vs_am_e", "vs_am_l")
corr_table <- cbind (position="d1_d2", DE_source=rep("gg_l", 2), comparison=c("random", "orthologs"), corr_table)
write.table(corr_table,file="output_files/corr_table_orth_vs_random_amggmm.txt", append=TRUE, row.names=FALSE, col.names=FALSE)
