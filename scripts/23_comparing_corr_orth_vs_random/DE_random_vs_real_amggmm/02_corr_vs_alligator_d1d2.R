...
for(k in 1:length(limb)){
  ...
  target_list <- merge(TPM_withID, one_to_one_am_12, by="am_gene_id") # these are TPMs of the 92 DE genes.
...
corr_table
write.table(corr_table, file="output_files/corr_table_orth_vs_random_amggmm.txt", row.names=FALSE)
...
for(k in 1:length(limb)){
  ...
  target_list_fold <- foldchange_lists[[5]]$am_gene_id %in% one_to_one_am_12$am_gene_id
...
corr_table <- matrix(unlist(simulated_gene_matrix), nrow=2, ncol=5)
colnames(corr_table) <- c("vs_mm", "vs_gg_e", "vs_gg_l", "vs_am_e", "vs_am_l")
corr_table <- cbind (position="d1_d2", DE_source=rep("am_l", 2), comparison=c("random", "orthologs"), corr_table)
corr_table
