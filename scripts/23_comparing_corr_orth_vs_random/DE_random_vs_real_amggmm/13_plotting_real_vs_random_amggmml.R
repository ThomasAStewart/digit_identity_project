...
# load data ####
correlation_table <- read.table("output_files/corr_table_orth_vs_random_amggmm_copy.txt", header = T) # both alligator early and late

position <- c("d1_d2", "d2_d3", "d3_d4")
...
for(k in 1:length(position)){
  ...
  real_dist <- as.vector(rbind(real_dist[,4], real_dist[,5], real_dist[,6], real_dist[,7], real_dist[,8] )) #, real_dist[,7]))
  random_dist <- as.vector(rbind(random_dist[,4], random_dist[,5], random_dist[,6], random_dist[,7], random_dist[,8] )) #, random_dist[,7]))
