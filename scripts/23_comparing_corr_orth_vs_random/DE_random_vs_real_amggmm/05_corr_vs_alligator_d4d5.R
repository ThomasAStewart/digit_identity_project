...
for(k in 1:length(limb)){
  ...
  TPM_lists <- data.frame(TPM_mm_4,
                          "TPM_gg_HL_4_e"= TPM_gg$gg_HL_D4_E,
                          "TPM_gg_HL_4_l"= TPM_gg$gg_HL_D4_L,
                          TPM_am_4_e, TPM_am_4_l)
  ...
  foldchange_lists <- list(data.frame(tpm_fold_change_mm[,1:2], tpm_fold_change_mm["change4v5"]),
                           data.frame(tpm_fold_change_gg[,1:2], tpm_fold_change_gg["change3v4_e"]),
                           data.frame(tpm_fold_change_gg[,1:2], tpm_fold_change_gg["change3v4_l"]),
  ...
  target_list <- merge(TPM_withID, one_to_one_am_45, by="am_gene_id") # these are TPMs of the 92 DE genes.
  target_list <- target_list[order(target_list$gene_symbol),]
  
  target_list_TPM <- target_list$TPM_am_4_e
  target_list_fold <- foldchange_lists[[4]]$am_gene_id %in% one_to_one_am_45$am_gene_id
  ...
  # real
  real_fold_list <- merge(one_to_one_am_45, foldchange_lists[[k]], by=taxonid, sort=F)
...
corr_table <- cbind (position="d4_d5", DE_source=rep("am_e", 2), comparison=c("random", "orthologs"), corr_table)
...
# repeat for late state alligator ####
limb <- c("mm",  "gg_e", "gg_l",  "am_e", "am_l")
simulated_gene_matrix <- vector("list",length(limb))
...
for(k in 1:length(limb)){
  ...
  target_list_TPM <- target_list$TPM_am_4_l
  target_list_fold <- foldchange_lists[[5]]$am_gene_id %in% one_to_one_am_45$am_gene_id
  target_list_fold <- foldchange_lists[[5]][target_list_fold,]
...
corr_table <- matrix(unlist(simulated_gene_matrix), nrow=2, ncol=5)
colnames(corr_table) <- c("vs_mm", "vs_gg_e", "vs_gg_l", "vs_am_e", "vs_am_l")
corr_table <- cbind (position="d4_d5", DE_source=rep("am_l", 2), comparison=c("random", "orthologs"), corr_table)
write.table(corr_table,file="output_files/corr_table_orth_vs_random_amggmm.txt", append=TRUE, row.names=FALSE, col.names=FALSE)
