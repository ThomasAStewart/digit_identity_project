...
print(g)
ggsave("output_figures/extended_fig_1_a_mm_allgenes_pca.pdf", width = 6, height = 6, units=c("cm"))
...
supplementary_table <- data.frame(rep("extend_data_1_a_mouse", length(pca$x[,1])), row.names(pca$x), pca$x[,1], boots[,1], pca$x[,2], boots[,2])
colnames(supplementary_table) <- c("plot", "sample", "PC1_loading", "PC1_SE", "PC2_loading", "PC2_SE")
write.table(supplementary_table, file = "~/Desktop/digit_RNA_seq/digit_project/output_files/extend_data_1_a_mouse.txt", sep = "\t", row.names = FALSE, col.names = TRUE)


# HCA ####
pdf("output_figures/extended_fig_1_a_mm_allgenes_clust.pdf")
hc <- pvclust(data, method.hclust="average"
              , method.dist="correlation", nboot=1000)
plot (hc, print.pv=TRUE, print.num=FALSE, main="mouse - all genes")
dev.off()
...
x <- as.vector(c(1, 2, 3,
                 ...
                 12,
                 14, 15, 
                 10, 13)) # this is the re-ordered pattern to correspond to HCA with rotations for readability.
...
print(q)
ggsave("output_figures/extended_fig_1_a_mm_allgenes_heat_ordered.pdf", width = 8, height = 8, units=c("in"))